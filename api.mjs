import { ATTACHMENT_TYPES, MAX_ATTACHMENTS, MAX_ATTACHMENT_SIZE, MAX_ATTACHMENTS_TOTAL } from './src/attachments.js';
...
// Whitelist used both by the chat payload check and by the server-side settings storage.
export function normalizeProviderList(value) {
  ...
  return value.map((p, index) => {
    ...
    if (p.type === 'custom') {
      let url;
      try { url = new URL(p.baseUrl); } catch { throw new Error('Проверьте URL своего провайдера.'); }
...
export function validatePayload(data) {
  ...
  for (const p of providers) {
    if (!providerTypes.includes(p.type) ||
        typeof p.model !== 'string' || !p.model.trim() || p.model.length > 200 ||
...
// Attachments only carry metadata in the payload; the server resolves ids into text or base64 once per request.
async function resolveAttachments(messages, readAttachment) {
  ...
  return Promise.all(messages.map(async m => {
    ...
    for (const a of m.attachments) {
      if (!cache.has(a.id)) {
        cache.set(a.id, await Promise.resolve(readAttachment ? readAttachment(a.id) : null).catch(() => null));
      }
      const found = cache.get(a.id);
      files.push(found
        ? { name: a.name, mime: a.type, text: found.text ?? null, base64: found.base64 ?? null }
        : { name: a.name, missing: true });
    }
    return { ...m, files };
  }));
}
...
export function explainFailure(status, payload, secrets = []) {
  ...
  // Redact before truncating so a cut-off credential cannot escape masking.
  for (const secret of secrets.filter(Boolean).sort((a, b) => b.length - a.length)) {
    detail = detail.split(secret).join('[ключ скрыт]');
  }
    ...
    for (const [keyIndex, key] of keys.entries()) {
      ...
      try {
        ...
        const response = await fetchImpl(request.url, {
          ...
          signal: attemptSignal, redirect: 'error',
        });
        if (!response.ok) {
        ...
        if (typeof content !== 'string' || !content.trim()) {
          report({ type: 'failure', provider: label, model: p.model, key: keyIndex + 1, reason: 'Провайдер вернул пустой ответ. Возможны ограничение содержимого или неподдерживаемый формат ответа.' });
          break;
        }
        ...
        const reason = error?.name === 'TimeoutError' ? `Истекло время ожидания ответа (${Math.round(timeoutMs / 1000)} секунд)`
          ...
          : ['ENOTFOUND', 'EAI_AGAIN'].includes(code) ? 'Не удалось найти сервер API (DNS). Проверьте адрес и сеть.'
          : ['CERT_HAS_EXPIRED', 'UNABLE_TO_VERIFY_LEAF_SIGNATURE', 'DEPTH_ZERO_SELF_SIGNED_CERT'].includes(code) ? 'Ошибка сертификата HTTPS сервера API'
          : code === 'ECONNREFUSED' ? 'Сервер API отказал в соединении'
          : ['EACCES', 'EPERM'].includes(code) ? 'Среда запуска запрещает сетевой доступ серверу ARIS. Запустите start.cmd в обычном терминале Windows.'
          : 'Не удалось связаться с API. Проверьте сеть, прокси и доступность адреса.';
